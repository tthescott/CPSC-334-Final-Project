...
public class PhysicsPanel extends SimulationPanel {
	...
	private void setupCollisionListener(PlinkoGame plinkoGame){
		...
		CollisionListener<SimulationBody, BodyFixture> cl = new CollisionListenerAdapter<SimulationBody, BodyFixture>() {
			...
			// if the ball collides with the floor then the floor will change to the same color of the ball
			private void checkHitFloor(SimulationBody ball, SimulationBody body, SimulationBody floor) {
			...
			// if the ball collides with a spoke then the spoke will change to same color as the ball
			private void checkHitSpokes(SimulationBody ball, SimulationBody body, ArrayList<SimulationBody> spokes) {
	...
	public void sendBall(Color color, Double balance) {
		...
		ball.translate(getBallDropPosition()); // issue with ball getting stuck if hitting spoke dead on. Offset can be as small as .01
        world.addBody(ball);
		this.ballCtr++;
	}
